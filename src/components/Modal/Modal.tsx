...
const Modal: FC<ModalProps> = ({ title, className, titleClassName, onClose, onProcess, children, ...props }) => {
        ...
        >
          <div className='relative flex flex-col max-h-full'>
            <div className='flex items-center text-white'>
              {
                title?.length &&
                <h3 className={`text-3xl font-[700] font-mono flex-1 text-center px-12 ${titleClassName}`}>{title}</h3>
              }
              <CrossIcon
